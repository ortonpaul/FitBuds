...
dataframe = pd.read_csv("data/training_backwards.csv")
print(dataframe)
...
testingdata = pd.read_csv("data/testing_backwards.csv")
XTestingData = testingdata.drop(['Label'], axis=1)

print(XTestingData)
print(list(XTestingData))

print(clf.predict([XTestingData.loc[1, : ]]))

import coremltools
coreml_model = coremltools.converters.sklearn.convert(clf)
coreml_model.save('backwards_model.mlmodel')